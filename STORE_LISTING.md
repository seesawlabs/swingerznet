# Store listing text

Ready-to-paste copy for App Store Connect and Play Console. Full interactive
version with live character counts: https://claude.ai/code/artifact/7ad54e1b-2214-4fd0-9cdb-f32f08cd42fe

## Apple App Store

**App name** (8/30)
```
Swingerz
```

**Subtitle** (28/30)
```
Tennis & pickleball, matched
```

**Promotional text** (141/170) — editable without a new build
```
Austin's invite-only beta is open. Join a group, post when you're free, and get matched by skill — pickleball or tennis, no more group texts.
```

**Keywords** (98/100) — comma-separated, no spaces after commas
```
pickleball,tennis,pickleball partner,tennis partner,doubles,ntrp,racquet sports,austin,padel,squad
```

**Copyright line**
```
© 2026 Swingerz
```

## Google Play

**App name** (8/30)
```
Swingerz
```

**Short description** (73/80)
```
Pickleball & tennis crews — post your availability, get matched by skill.
```

**Category**
```
Sports
```

**Tags** — Play selects from its own controlled list, not free text
```
Pickleball, Tennis, Racquet sports
```

**Contact details**
```
Email:   love@swingerz.net
Website: swingerz.net
Phone:   optional — none listed yet
```

## Description / Full description (1164/4000)

Shared between both stores — Apple's Description and Play's Full description
both accept this plain text and line breaks as-is.

```
Stop texting six people to find one who's free.

Swingerz helps pickleball and tennis players in Austin build real, recurring crews — then makes it easy to actually get on court together.

GROUPS
Create or join a group with people who play like you do — casual doubles on Tuesday mornings, a competitive 3.0–4.0 group, or your regular crew from the courts at Zilker. Invite friends, or find an established group looking for players.

AVAILABILITY
Post an open window and let your group know you're free. See who else has posted, propose a time, and confirm — no more "who's around Saturday?" text chains that go nowhere.

MATCHES
Get matched with players at your level — NTRP rating for tennis, skill band for pickleball — from casual rallies to competitive sets. Confirm, decline, or watch a proposal move through your group in real time.

Built for Austin players first, at courts you already know: Zilker, Caswell, Pharr, Northwest Rec, Austin Pickle Ranch, Bartholomew, Pan American, and more.

Swingerz is free, with no ads and nothing to buy. It's not a booking platform — grab your own court, we just help you find who to play with.

Connect. Match. & Play.
```

---

Reflects what's actually built and locked so far — Austin-only launch, free
with no ads or in-app purchases, groups/availability/matches as the three
core loops. Update the promotional text and description together once the
beta status or city scope changes.
